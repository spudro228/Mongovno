...
class BsonIterator implements \Iterator
{
    ...
    private function initCurrent()
    {
        ...
        if ($this->bufferLength - $this->bufferOffset < 5) {
            throw new \RuntimeException(sprintf('Expected at least 5 bytes; %d remaining', $this->bufferLength - $this->bufferOffset));
        }
        //первые 4 байта длинна фрагмента
        [, $documentLength] = unpack('V', substr($this->buffer, $this->bufferOffset, 4));
